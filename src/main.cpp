...
int main()
{
  	...
  	// Parsing the formula
  	LexerTree formulaLexerTree("5* (-    x+(((4))))");

  	ParserOperatorPriorities pop;
  	ExpressionParser formulaParser(pop);

	ParserNode* formulaRoot = formulaParser.parse(formulaLexerTree.getRoot(), vars);
	...
	vars.generateLLVMVariableSetValueCode("x", 10.1, builder);
